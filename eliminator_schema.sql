...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_full_records_unsummed`
AS select
   `v_schedule_and_results`.`game_id` AS `game_id`,
   `v_schedule_and_results`.`season` AS `season`,
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_full_records`
AS select
